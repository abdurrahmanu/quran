...
const {surahNames, surahsArabic, surahsEnglish, currentSurah} = storeToRefs(quran)
...
<template>
  <div v-if="surahNames.length">
    <div class="flex flex-col h-[100%] selection:bg-none">
      <Head />
      <div :class="[language === 'en' ? 'flex-row' : 'flex-row-reverse']" class="max-w-[1100px] max-h-[100dvh] overflow-y-auto mx-auto shadow-md shadow-black w-[100%] flex">
        <SurahNames/>
        <div class="sm:w-[80%] bg-neutral-900 h-full overflow-y-auto w-[100%]">
          <Surah />
        </div>
      </div>
    </div>
